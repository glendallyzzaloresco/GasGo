...
<style>
    ...
    .section-tabs {
        display:flex; gap:12px; margin-bottom:28px; border-bottom:2px solid #e0e0e0; flex-wrap:wrap;
    }
    .section-tab {
...
<div class="section-tabs">
    ...
    <button class="section-tab" data-section="freebies" onclick="switchSection('freebies', this)">
        ...
        <span class="badge bg-warning text-dark rounded-pill ms-2" style="font-size:0.75rem;">{{ $freebieCatalog->count() }}</span>
    </button>
    <button class="section-tab" data-section="archived" onclick="switchSection('archived', this)">
        ...
        <span class="badge bg-secondary rounded-pill ms-2" style="font-size:0.75rem;" id="archivedTabCount">{{ $archivedCatalog->count() }}</span>
    </button>
</div>
...
<div id="productsSection" class="section-content">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            ...
            <p class="text-muted mb-0" style="font-size:.88rem;">Manage active products that customers can view and purchase</p>
        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex gap-2">
                        @if($isProductFreebie)
                            ...
                            ><i class="fas fa-edit me-1"></i>Edit</button>
                            <form action="{{ route('admin.products.destroy', $freebie) }}" method="POST" onsubmit="return confirm('Delete this item? If it is linked to past orders, it will be moved to Archived Products safely.');">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm" type="submit" style="background:#f8d7da;color:#dc3545;border-radius:8px;" title="Delete"><i class="fas fa-trash"></i></button>
                            </form>
...
<div id="archivedSection" class="section-content" style="display:none;">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            <h5 class="fw-bold" style="color:#475569;margin-bottom:6px;"><i class="fas fa-archive me-2 text-secondary"></i>Archived Products & Items</h5>
            <p class="text-muted mb-0" style="font-size:.88rem;">Products that are preserved to protect historical customer orders and sales logs. You can restore them to active anytime.</p>
        </div>
        <div class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm" style="width:260px;">
                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                <input type="text" id="searchArchived" class="form-control border-start-0" placeholder="Search archived..." onkeyup="filterArchived()">
            </div>
        </div>
    </div>
    ...
    <div class="row g-4" id="archivedGrid">
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 archived-item" data-name="{{ strtolower($item['name']) }}">
            <div class="product-card archived-card" style="position:relative;">
                ...
                @else
                    <div style="width:100%;height:240px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;">
                ...
                <div class="card-body">
                    ...
                    <p class="text-muted mb-2 text-truncate" style="font-size:.82rem;">{{ $item['description'] ?? 'No description' }}</p>
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="price" style="color:#64748b;font-size:1.05rem;">₱{{ number_format((float) ($item['price'] ?? 0), 2) }}</span>
                        <span class="badge bg-light text-dark border" style="font-size:.72rem;">{{ ucfirst($item['category']) }}</span>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="d-flex gap-2">
                        @if($item['type'] === 'product')
                            <button
                                ...
                                onclick="openRestoreProductModal('{{ addslashes($item['name']) }}', '{{ $item['restore_url'] }}')"
                            ><i class="fas fa-undo me-1"></i>Bring Back</button>
                            @if($item['orders_count'] === 0)
                                <form action="{{ $item['delete_url'] }}" method="POST" onsubmit="return confirm('Permanently delete this product? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-sm" type="submit" style="background:#f8d7da;color:#dc3545;border-radius:8px;" title="Permanently Delete"><i class="fas fa-trash"></i></button>
                                </form>
                            @else
                                <button class="btn btn-sm" type="button" style="background:#f1f5f9;color:#94a3b8;border-radius:8px;cursor:not-allowed;" title="Protected: This product is part of previous customer orders and cannot be permanently removed." disabled><i class="fas fa-lock"></i></button>
                            @endif
                        @else
                            <form action="{{ $item['restore_url'] }}" method="POST" class="flex-grow-1">
                                @csrf
                                <button class="btn btn-sm w-100" style="background:var(--gasgo-blue);color:#fff;border-radius:8px;font-weight:600;" type="submit">
                                    <i class="fas fa-undo me-1"></i>Bring Back
                                </button>
                            </form>
                            <form action="{{ $item['delete_url'] }}" method="POST" onsubmit="return confirm('Permanently delete this freebie?');">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm" type="submit" style="background:#f8d7da;color:#dc3545;border-radius:8px;" title="Delete"><i class="fas fa-trash"></i></button>
                            </form>
        ...
        <div class="col-12">
            <div class="text-center py-5" style="background:#fff;border-radius:16px;border:1px dashed #cbd5e1;">
                ...
                <p class="text-muted mb-0" style="font-size:.85rem;">When products linked to previous orders are removed, they will be safely kept here.</p>
            </div>
        </div>
        @endforelse
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius:16px;">
            <div class="modal-header" style="border-bottom:none;padding:24px 24px 0;">
                <h5 class="modal-title fw-bold" style="color:var(--gasgo-blue);"><i class="fas fa-undo me-2"></i>Restore Product</h5>
            ...
            <form id="restoreProductForm" method="POST" action="">
                ...
                <div class="modal-body modal-form" style="padding:20px 24px;">
                    <p class="text-muted mb-3" style="font-size:.9rem;">
                        You are about to restore <strong id="restoreProductName" class="text-dark"></strong> back to active products so customers can view and purchase it.
                    </p>
                    <div class="mb-3">
                        <label class="mb-1">Starting Stock Quantity <small class="text-muted">(optional, enter current stock or leave 0)</small></label>
                        <input type="number" class="form-control" name="stock" id="restoreProductStock" placeholder="0" min="0" value="0">
                    </div>
                </div>
                <div class="modal-footer" style="border-top:none;padding:0 24px 24px;">
                    ...
                    <button type="submit" class="btn" style="background:var(--gasgo-blue);color:#fff;border-radius:10px;font-weight:600;padding:10px 24px;">
