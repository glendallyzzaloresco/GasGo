...
<style>
    ...
    .status-pill.good { background:rgba(39,174,96,.14); color:#1e8449; }
    .status-pill.warn { background:rgba(231,76,60,.14); color:#c0392b; }
    .status-pill.info { background:rgba(26,109,176,.12); color:#1a6db0; }
    .process-flow {
...
<div class="report-card mb-3">
    <div class="section-heading mb-0">
        <div>
            ...
            <p class="section-note">Delivered orders are used as sales history only through completed Inventory OUT transactions. Inventory stock is only used after forecasting to check restocking needs.</p>
        </div>
            ...
            <div class="table-responsive" style="max-height:300px;">
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="3" class="text-center text-muted py-3">No inventory OUT transactions found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="table-responsive" style="max-height:280px;">
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="3" class="text-center text-muted py-3">No customer sales data available.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="table-responsive" style="max-height:280px;">
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="3" class="text-center text-muted py-3">No daily sales data available.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<div class="report-card mb-3">
    <div class="section-heading">
        <div>
            ...
            <p class="section-note">Historical revenue only from Inventory OUT transactions. No forecast overlay in this chart.</p>
        </div>
    </div>
...
<div class="report-card mb-3" id="forecastSection">
    <div class="section-heading">
        <div style="flex:1">
            <h6 class="fw-bold mb-1" style="color:var(--gasgo-blue);">Section 2: Forecast Analytics (Triple Exponential Smoothing)</h6>
            <p class="section-note">Forecast generated from Inventory OUT transactions only. Pending, cancelled, and incomplete deliveries are excluded.</p>

            <form id="forecastForm" method="GET" action="{{ route('admin.reports') }}" class="row g-3 align-items-end mt-3">
                ...
                <div class="col-lg-3 col-md-6">
                    ...
                    <select class="form-select form-select-sm" name="forecast_period" id="forecastPeriodSelect">
                        <option value="next_7_days" {{ $forecastPeriod == 'next_7_days' ? 'selected' : '' }}>Next 7 Days</option>
                        <option value="next_30_days" {{ $forecastPeriod == 'next_30_days' ? 'selected' : '' }}>Next 30 Days</option>
                        <option value="next_month" {{ $forecastPeriod == 'next_month' ? 'selected' : '' }}>Next Month</option>
                    </select>
                </div>
        ...
        <span class="insight-chip" id="forecastInsightChip"><i class="fas fa-filter"></i>{{ $forecastPeriodLabel }} for {{ optional($productsForForecast->firstWhere('id', $forecastProductId))->name }}</span>
    </div>
    ...
    <div class="row g-3">
        ...
        <div class="col-lg-4">
            <div class="summary-row"><span>Forecast Demand</span><strong id="forecastDemandValue">{{ number_format($forecastSummary['forecast_demand'], 2) }} Units</strong></div>
            <div class="summary-row"><span>Forecast Period</span><strong id="forecastPeriodValue">{{ $forecastSummary['forecast_period'] }}</strong></div>
            <div class="summary-row"><span>Expected Growth</span><strong id="expectedGrowthValue">{{ $forecastSummary['expected_growth'] >= 0 ? '+' : '' }}{{ number_format($forecastSummary['expected_growth'], 1) }}%</strong></div>
            <div class="summary-row"><span>Forecast Accuracy</span><strong id="forecastAccuracyValue">{{ number_format($forecastSummary['forecast_accuracy'], 1) }}% (MAPE {{ number_format($forecastSummary['mape'], 2) }})</strong></div>
            <div class="section-divider"></div>
            <div class="summary-row"><span>Level</span><strong id="forecastLevelValue">{{ number_format($forecastModel['level'] ?? 0, 2) }}</strong></div>
            <div class="summary-row"><span>Trend</span><strong id="forecastTrendValue">{{ number_format($forecastModel['trend'] ?? 0, 2) }}</strong></div>
            <div class="summary-row"><span>Seasonality Avg</span><strong id="forecastSeasonalityValue">{{ number_format($forecastModel['seasonalityAverage'] ?? 0, 2) }}</strong></div>
        </div>
    </div>
</div>
            ...
            <div class="table-responsive" style="max-height:280px;">
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="3" class="text-center text-muted py-3">No seasonal pattern detected yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<div class="row g-3 mb-3">
    <div class="col-lg-5">
        <div class="report-card h-100">
            ...
            <div class="summary-row"><span>Trending Product</span><strong>{{ $forecastInsights['trending_product'] }}</strong></div>
        ...
        <div class="report-card h-100">
            <div class="section-heading">
                <div>
                    ...
                    <p class="section-note">How Holt-Winters turns historical inventory movements into restocking guidance.</p>
                </div>
            </div>
            <div class="process-flow">
                <div class="process-item">
                    <div class="process-title">Historical Sales (Inventory OUT)</div>
                    <div class="process-text">Completed Inventory OUT transactions linked to completed/delivered orders.</div>
                </div>
                <div class="process-item">
                    <div class="process-title">Triple Exponential Smoothing</div>
                    <div class="process-text">Level: current average demand. Trend: upward/downward direction. Seasonality: recurring demand cycles.</div>
                </div>
                <div class="process-item">
                ...
                <div class="process-item">
                    ...
                    <div class="process-text">Compare forecast demand with stock-on-hand and recommend restock actions.</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="table-responsive" style="max-height:260px;">
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="5" class="text-center text-muted py-3">No restock recommendations available.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
