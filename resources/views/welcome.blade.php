...
@section('content')
        ...
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-6 hero-content" data-aos="fade-right" data-reveal="slide-up">
                    <h1 class="hero-title">{{ $homepageSettings->hero_title_prefix ?? 'Fast, Reliable' }}
                        <span>{{ $homepageSettings->hero_title_highlight ?? 'LPG Delivery' }}</span>
                        {{ $homepageSettings->hero_title_suffix ?? 'to Your Door' }}</h1>
                    <p class="hero-subtitle">
                        {{ $homepageSettings->hero_subtitle ?? 'Fast, reliable LPG delivery right to your door. Earn loyalty & promos with every order.' }}
                    </p>
                    <a href="{{ url('/customer/product') }}" class="btn btn-gasgo btn-lg">
                        <i
                            class="fas fa-fire me-2"></i>{{ $homepageSettings->hero_primary_button_label ?? 'Browse Products' }}
                    </a>
    ...
    @guest
        <section class="container" style="position:relative;z-index:2;" data-aos="fade-up">
            <div class="promo-banner" @if($promoBannerImage)
                style="background-image: linear-gradient(rgba(247,148,29,.82), rgba(255,107,53,.82)), url('{{ $promoBannerImage }}'); background-size: cover; background-position: center;"
            @endif>
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h3><i
                    ...
                    <div class="col-md-4 text-md-end mt-3 mt-md-0">
                        <a href="{{ url('/customer/loginRegistration?tab=register') }}" class="btn btn-light btn-lg"
                            style="border-radius:25px;font-weight:600;color:var(--gasgo-orange);">
                            {{ $homepageSettings->promo_button_label ?? 'Register Now' }} <i
        ...
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-up">
                ...
                <p class="section-subtitle">
                    {{ $homepageSettings->products_section_subtitle ?? 'Choose from our range of LPG tanks and accessories' }}
                </p>
            </div>
            <div class="row g-4">
                    ...
                    <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}">
                        <div class="product-card">
                            <div class="product-img">
                                ...
                                @if($product->category)
                                    <span
                                        class="product-badge {{ strtolower($product->category) === 'accessories' ? 'accessory' : '' }}">{{ $product->category }}</span>
                                @endif
                            </div>
                            <div class="product-body">
                                <h5>{{ $product->name }}</h5>
                                @if($product->description)
                                    <p class="product-variant mb-2"
                                        style="font-size:.85rem; color:#555; margin-bottom:8px; font-weight:500;">
                                        {{ $product->description }}</p>
                                @endif
                                <p class="product-stock {{ $inStock ? '' : 'out' }}">
                                    <i
                                        class="fas {{ $inStock ? 'fa-check-circle' : 'fa-times-circle' }} me-1"></i>{{ $inStock ? 'In Stock' : 'Out of Stock' }}
                                </p>
                                ...
                                @auth
                                    <div class="product-actions" style="flex-direction: column;">
                                        <a href="{{ route('customer.product.show', $product->id) }}" class="btn btn-gasgo btn-sm"
                                            style="text-decoration: none; width: 100%; text-align: center; padding: 10px 15px; font-weight: 600;">
                                ...
                                @else
                                    <div class="product-actions" style="flex-direction: column;">
                                        ...
                                        <button class="btn-add add-to-cart-btn-welcome" data-id="{{ $product->id }}"
                                            data-name="{{ $product->name }}" data-price="{{ $product->price }}"
                                            data-image="{{ $img }}" {{ $inStock ? '' : 'disabled' }} title="Add to Cart">
            ...
            <div class="text-center mt-5" data-aos="fade-up">
                <a href="{{ url('/customer/product') }}" class="btn btn-gasgo-outline btn-lg">
                    {{ $homepageSettings->products_view_all_label ?? 'View All Products' }} <i
                        class="fas fa-arrow-right ms-2"></i>
                </a>
            </div>
        </div>
        ...
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-up">
                <h2 class="section-title">
                    {{ $homepageSettings->why_choose_title ?? ('Why Choose ' . trim(($homepageSettings->brand_name_primary ?? 'Gas') . ' ' . ($homepageSettings->brand_name_accent ?? 'Go')) . '?') }}
                </h2>
                <p class="section-subtitle">
                    {{ $homepageSettings->why_choose_subtitle ?? 'We make delivery convenient, safe, and rewarding' }}</p>
            </div>
            <div class="row g-4">
                <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="100">
                    <div class="why-card text-center">
                        ...
                        <p class="text-muted small">Get your {{ strtolower($homepageSettings->industry_noun ?? 'order') }}
                            delivered fast</p>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="400">
                    <div class="why-card text-center">
                        ...
                        <p class="text-muted small">Certified {{ $homepageSettings->industry_noun ?? 'quality' }} products
                            with guaranteed safety</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                @empty
                    <div class="col-12 text-center py-4">
                        <div class="p-4 bg-light rounded-4 border d-inline-block" style="max-width: 520px;">
                            ...
                            <p class="text-muted small mb-0">Customer delivery reviews and ratings will be displayed here once submitted.</p>
                        </div>
                    </div>
                @endforelse
            </div>
